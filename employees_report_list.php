...
list($from_year,$from_month,$from_day) = (explode("-",$_COOKIE["employeesfromdate"]));
$from_date = $from_day.'-'.$from_month.'-'.$from_year;

list($to_year,$to_month,$to_day) = (explode("-",$_COOKIE["employeestodate"]));
$to_date = $to_day.'-'.$to_month.'-'.$to_year;
$EmployeesReport = getEmployeesReport($_COOKIE["employeesfromdate"],$_COOKIE["employeestodate"]);
