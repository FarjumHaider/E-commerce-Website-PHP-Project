...
list($from_year,$from_month,$from_day) = (explode("-",$_COOKIE["productfromdate"]));
$from_date = $from_day.'-'.$from_month.'-'.$from_year;

list($to_year,$to_month,$to_day) = (explode("-", $_COOKIE["producttodate"]));
$to_date = $to_day.'-'.$to_month.'-'.$to_year;
$ProductReport = getProductReport($_COOKIE["productfromdate"],$_COOKIE["producttodate"]);
