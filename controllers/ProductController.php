...
require_once 'models/db_config.php';
  ...
  if(isset($_POST["add_product"]))
  {
    ...
    if(!$hasError){
      ...
      $rs=insertProduct($pname,$c_id,$price,$quantity,$description,$store_date,$p_image);
      if ($rs === true) {
...
//------------------
  ...
  if(isset($_POST["edit_product"]))
  {
    ...
    if(!$hasError){
      $store_date= $year.'-'.$month.'-'.$day;
      $rs=updateProduct($pname,$c_id,$price,$quantity,$description,$store_date,$p_image,$_POST["id"]);
      if ($rs === true) {
        header("Location: all_products.php");
      }
      $err_db = $rs;
  ...
  $query="select p.*,c.name as 'c_name' from products p left join categories c on p.c_id=c.id order by store_date asc";
  $rs=get($query);
  return $rs;
}
