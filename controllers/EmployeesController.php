...
else if (isset($_POST["delete_employees"])){
  //do validations
  //if no error
  $rs = deleteEmployees($_POST["id"]);
  if($rs === true){
    header("Location: employees_list.php");
    // header("Location: controllers/LoginController.php");
  }
...
function getAllEmployees(){
  $query="select * from users order by joining_date asc";
  $rs=get($query);
  return $rs;
}
...
function updateUsers($fname,$lname,$uname,$pass,$email,$phone_num,$joining_date,$gender,$role,$salary,$address,$img,$id){

  $query="update users set f_name='$fname',l_name='$lname',username='$uname',password='$pass',email='$email',phone=$phone_num,joining_date='$joining_date',gender='$gender',role='$role',salary=$salary,address='$address',image='$img'  where id = $id";
