...
elseif(strpos($_POST["username"]," "))
  ...
  if(!$hasError){
    ...
    $rs=insertPayingSalary($fname,$lname,$uname,$salary,$amount,$due,$payment_date);
    if ($rs === true) {
...
function insertPayingSalary($fname,$lname,$uname,$salary,$amount,$due,$payment_date){
  $query="insert into salary values (NULL,'$fname','$lname','$uname',$salary,$amount,$due,'$payment_date')";
...
function getAllPaidEmployees(){
  $query="select * from salary order by payment_date asc";
  $rs=get($query);
  return $rs;
}
...
function searchDueSalary($key){
$query = "select * from salary where id like '$key'";
$rs = get($query);
return $rs;
// getDueSalary
}
