...
list($from_year,$from_month,$from_day) = (explode("-",$_COOKIE["salaryfromdate"]));
$from_date = $from_day.'-'.$from_month.'-'.$from_year;

list($to_year,$to_month,$to_day) = (explode("-",$_COOKIE["salarytodate"]));
$to_date = $to_day.'-'.$to_month.'-'.$to_year;
$SalaryReport = getSalaryReport($_COOKIE["salaryfromdate"],$_COOKIE["salarytodate"]);
